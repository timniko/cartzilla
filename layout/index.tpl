        ...
        {block name='layout-index-content'}
            ...
            {block name='layout-index-link-content'}
                {if !empty($Link->getContent())}
                    ...
                    {container fluid=$Link->getIsFluid() class="link-content{if $Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive }container-plus-sidebar{/if}{if $Link->getIsFluid()} px-0{/if}"}
                        {$Link->getContent()}
                    {/container}
                {/if}
            {/block}
