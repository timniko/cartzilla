{block name='layout-header'}
    ...
    <head>
        ...
        <script src="{$ShopURL}/templates/cartzilla/js/jquery-3.5.1.min.js"></script>
        <script src="{$ShopURL}/templates/cartzilla/js/lazysizes.min.js"></script>
        ...
        {if !empty($oUploadSchema_arr)}
            <script defer src="{$ShopURL}/templates/cartzilla/js/fileinput/fileinput.min.js"></script>
            <script defer src="{$ShopURL}/templates/cartzilla/js/fileinput/themes/fas/theme.min.js"></script>
            <script defer src="{$ShopURL}/templates/cartzilla/js/fileinput/locales/{$uploaderLang}.js"></script>
        {/if}
        {if $Einstellungen.preisverlauf.preisverlauf_anzeigen === 'Y' && !empty($bPreisverlauf)}
            <script defer src="{$ShopURL}/templates/cartzilla/js/Chart.bundle.min.js"></script>
        {/if}
    ...
    {block name='layout-header-body-tag'}
      <body class="{if $Einstellungen.template.theme.wish_compare_animation === 'mobile'
       ...
       {if $nSeitenTyp === $smarty.const.PAGE_BESTELLVORGANG} is-checkout{/if} is-nova handheld-toolbar-enabled"
        ...
        {if isset($cz_config->cz_color1) && !empty($cz_config->cz_color1)}style="background-color: {$cz_config->cz_color1};"{/if}>
    ...
    {/block}
      ...
      {if $bAdminWartungsmodus}
        {block name='layout-header-maintenance-alert'}
          <div class="alert alert-warning fade show alert-dismissible m-0" role="alert">
          	<span class="fw-medium"><i class="ci-security-announcement me-2"></i></span>{lang key='adminMaintenanceMode'}
      ...
      {if $smarty.const.SAFE_MODE === true}
        {block name='layout-header-safemode-alert'}
          <div class="alert alert-warning fade show alert-dismissible m-0" role="alert">
          	<span class="fw-medium"><i class="ci-security-announcement me-2"></i></span>{lang key='safeModeActive'}
						<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
          </div>
        {/block}
      {/if}
