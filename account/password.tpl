{block name='account-password'}
    ...
    {block name='account-password-content'}
        ...
        {/block}
        {container fluid=$Link->getIsFluid() class="py-4 py-lg-5 my-4 account-password {if $Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive}container-plus-sidebar{/if}"}
            {if $step === 'formular'}
                {row class="justify-content-center"}
                    {col cols=12 md=10 lg=8}
                        ...
                        {block name='account-password-heading'}
                          <h1 class="h3 mb-4">{lang key='forgotPassword' section='global'}</h1>
                        {/block}
